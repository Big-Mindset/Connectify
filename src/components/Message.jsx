...
const Message = React.memo(({ content, time, senderId, status,image }) => {
  ...
  return (
    <motion.div
      ...
      <div className={` ${image ? "md:max-w-[50%] max-w-[30%]" : "max-w-[70%] " } flex flex-col gap-1.5 overflow-hidden rounded-md font-[300]  px-[5px] py-[4px] ${senderId === session?.user.id ? ' bg-blue-800/80 ' : 'bg-gradient-to-br from-indigo-900/30 via-[#0F0A1D] to-[#0F0A1D]'}`}>
        ...
        <div className="flex flex-col   gap-1.5">
          ...
          <div className="flex items-center justify-between  self-end  gap-[3px]  flex-shrink-0">
            <p className="text-[0.6rem]  text-gray-100  font-[200] ">{newtime.toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit",
              hour12: true
            })} </p>
