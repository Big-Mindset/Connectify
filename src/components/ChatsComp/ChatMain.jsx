...
import { getmoremessagesIndex } from '@/database/indexdb'
...
const ChatMain = () => {
  ...
  let [haslocalDb , sethaslocalDb] = useState(true)
  const el = container.current;

  
  const scrolledContent = useRef((el?.scrollHeight - el?.scrollTop - el?.clientHeight) >= 800)
  useEffect(() => {
    ...
    let createdAt = topMessage?.getAttribute("data-createdat");
    ...
    let intersectionObserver = new IntersectionObserver(async (entries) => {
      if (entries[0].isIntersecting && !intersecting.current) {
        ...
        try {
          let res;
          if (haslocalDb){
            res = await getmoremessagesIndex(selectedInfo?.id , createdAt);
    ...
    if (topMessage instanceof Element){
  ...
  }, [messages.length, hasMore, loading,scrolledContent]);
